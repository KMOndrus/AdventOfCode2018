...
console.log(calWithForLoop(frequencyData));

function findFirstRepeatedAccumulatedFrequency(data) {
  data = data.split(' ');

  let currentAccumulatedFrequency = 0;
  let accumulatedFrequencySet = new Set();
  accumulatedFrequencySet.add(0);

  for (let i = 0; i < data.length; ++i) {
    if (data[i][0] === '+') {
      currentAccumulatedFrequency += Number(data[i].slice(1));
    } else {
      currentAccumulatedFrequency -= Number(data[i].slice(1));
    }
    if (accumulatedFrequencySet.has(currentAccumulatedFrequency)){
      console.log(currentAccumulatedFrequency);
      return currentAccumulatedFrequency;
    }
    accumulatedFrequencySet.add(currentAccumulatedFrequency);
    if (i === (data.length - 1)){
      i = -1;
    }
  }
...
findFirstRepeatedAccumulatedFrequency(sampleData1);
findFirstRepeatedAccumulatedFrequency(sampleData2);
findFirstRepeatedAccumulatedFrequency(sampleData3);
findFirstRepeatedAccumulatedFrequency(sampleData4);
findFirstRepeatedAccumulatedFrequency(frequencyData);
